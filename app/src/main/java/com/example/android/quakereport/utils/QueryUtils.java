...
public final class QueryUtils {
    ...
    private static final String JSON_FEATURES_NAME = "features";
    private static final String JSON_PROPERTIES_NAME = "properties";
    private static final String JSON_MAGNITUDE_NAME = "mag";
    private static final String JSON_PLACE_NAME = "place";
    private static final String JSON_TIME_NAME = "time";
    ...
    /**
     * Return a list of {@link com.example.android.quakereport.model.EarthQuake} objects that has been built up from
    ...
    public static ArrayList<EarthQuake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray features = root.getJSONArray(JSON_FEATURES_NAME);
            JSONObject jsonEarthQuake, jsonFeature;
            EarthQuake earthQuake;

            for(int i=0; i < features.length(); i++){
                jsonFeature = features.getJSONObject(i);
                jsonEarthQuake = jsonFeature.getJSONObject(JSON_PROPERTIES_NAME);
                double magnitude = jsonEarthQuake.getDouble(JSON_MAGNITUDE_NAME);
                long time = jsonEarthQuake.getLong(JSON_TIME_NAME);
                String place = jsonEarthQuake.getString(JSON_PLACE_NAME);
                String url = jsonEarthQuake.getString(JSON_URL_NAME);
                Date date = new Date();
                date.setTime(time);
                earthQuake = new EarthQuake(place, magnitude, date, url);
